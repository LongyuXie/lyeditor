...
/**
 * Rope Like文档数据模型
 * 使用平衡树优化对文档的拼接，分解，随机访问
 * 使用偏移量对文档进行随机访问，也可以直接访问行
 */
export abstract class Text implements Iterable<string> {
  ...
  abstract offsetAt(line: number, column: number): number
  positionAt(offset: number): [number, number]{
    let line = this.lineAt(offset);
    // TODO: 可以使用更加高效的算法, 而不是遍历两次平衡树
    let _offset = this.offsetAt(line.number, 0);
    return [line.number, offset - _offset];
  }
...
class TextLeaf extends Text {
  constructor(readonly text: readonly string[], readonly length: number = 0) {
    super()
    /**
     * TODO: 如果不进行检查, 会出现行数为0的错误(fixed)
     * 文档要求至少存在1行
     */
    if(text.length == 0){
      this.text = [""];
    }
    this.length = textLength(text);
  }
  ...
  offsetAt(line: number, column: number): number {
    ...
    let _offset = 0;
    for (let i = 0; i < l; i++){
      _offset += this.text[i].length + 1;
    }
    return _offset + column;
  }
...
class TextNode extends Text {
  ...
  offsetAt(line: number, column: number): number {
    ...
    for(let i = 0;; i++){
      ...
      if(l <= child.lines){
        /// 找到行号了
        _offset += child.offsetAt(l, column);
        break;
      }
      l -= child.lines;
      _offset += child.length + 1;
    }
    return _offset;
  }
  ...
  next(skip: number = 0): this {
    for (;;) {
      ...
      if (offset == (this.dir > 0 ? size : 0)) {
        ...
        // Internal offsetAt with lineBreak == false means we have to
